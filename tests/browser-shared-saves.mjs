...
const expandGame=async p=>{const title=gameCard(p).locator('.console-title');if(!await title.evaluate(e=>e.open))await title.locator('summary').first().click();};
...
const openLocation=async p=>{if(await p.locator('#panel').evaluate(e=>e.open))await p.locator('#closePanel').click();await p.locator('#savesButton').click();await p.getByRole('button',{name:'Save location',exact:true}).click();await p.waitForFunction(()=>!document.querySelector('#panelBody').textContent.includes('Checking shared saves'));};
try{
 const ca=await browser.newContext(),cb=await browser.newContext();for(const c of[ca,cb])await c.addInitScript(()=>localStorage.setItem('vnkit.settings',JSON.stringify({speed:0})));a=await ca.newPage();b=await cb.newPage();for(const p of[a,b])p.on('pageerror',e=>report.errors.push(e.message));
 await a.goto(`${base}/?game=${game}`);await ready(a);
 const file=path.resolve(process.env.VNKIT_CHECKPOINT||path.join(root,'private/clannad/menu-audit/checkpoints-final/source-portrait.json')),portrait=JSON.parse(await fs.readFile(file));assert.equal(portrait.state.pending.kind,'text','Use a stable source-text checkpoint');await a.locator('#saveFile').setInputFiles(file);await pollBrowser(async()=>(await local(a))?.state.pending?.id===portrait.state.pending.id,'local portrait restore');
 ...
 await a.locator('#libraryButton').click();await expandGame(a);await gameCard(a).getByRole('button',{name:'Route progress / debug…',exact:true}).click();await a.locator(`.route-progress[data-route="${route}"] button`).click();await pollBrowser(async()=>clearFlag(await local(a,'progress'))===true,'local manual-progress fixture');await a.locator('#closePanel').click();
 ...
 await a.locator('.slot').filter({hasText:/^slot 14 ·/}).getByRole('button',{name:'Save',exact:true}).click();await a.waitForFunction(()=>document.querySelector('#saveLocationStatus').classList.contains('error'));assert.equal(await a.locator('#nextButton').isDisabled(),true);assert.equal((await bank()).records.autosave.state.pending.id,newer.state.pending.id);assert.ok(await local(a,'shared-recovery'));pass('Stale first profile is paused before overwriting newer shared progress and retains a recovery copy');
 ...
 await expandGame(b);await gameCard(b).getByRole('button',{name:'Save location',exact:true}).click();await navigate(b,'Retry unsynced save');await ready(b);
