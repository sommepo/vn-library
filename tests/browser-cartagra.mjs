...
const unicode=process.env.VNKIT_UNICODE==='1';
const root=path.resolve(import.meta.dirname,'..'),library=path.resolve(process.argv[2]),out=path.resolve(process.argv[3]),game='cartagra-slpm66231-1.01';
...
try{
 ...
 for(let step=0;step<700&&(report.segments<150||report.choices<1);step++){
  await ready();
  ...
  if(saved.state.scene.music&&!report.musicDecoded){await page.waitForFunction(()=>[...window.testAudio].some(a=>a.src.includes('/BGM.AFS/')&&a.readyState>=2&&!a.paused&&a.currentTime>0),null,{timeout:20000});report.musicDecoded=true;}
  if(saved.state.pending.voice&&!report.voiceDecoded){await page.waitForFunction(()=>[...window.testAudio].some(a=>a.src.includes('/VOICE.AFS/')&&a.readyState>=2&&!a.paused&&a.currentTime>0),null,{timeout:20000});report.voiceDecoded=true;await page.screenshot({path:path.join(out,'voiced-scene.png')});}
  if(saved.state.pending.kind==='text'){
   if(unicode){
    assert.equal(await page.locator('#sentence canvas').count(),0);
    const text=await page.locator('#sentence').evaluate(el=>{const clone=el.cloneNode(true);clone.querySelectorAll('rt').forEach(r=>r.remove());return clone.textContent;});assert.equal(text,plainText(saved.state.pending.text));
    assert.equal(await page.locator('#speaker').textContent(),saved.state.pending.speaker);
  ...
  if(await page.locator('#choices button').count()){if(unicode)assert.deepEqual(await page.locator('#choices button').allTextContents(),saved.state.pending.options.map(o=>plainText(o.text)));else assert.ok(await page.locator('#choices button canvas').count());report.choices++;await page.screenshot({path:path.join(out,'original-choices.png')});await page.locator('#choices button').first().click();}
  else await page.locator('#nextButton').click();
 }
 assert.ok(report.segments>=150&&report.choices>=1&&report.movies>=2);pass(unicode?'150+ Unicode DOM segments, choices, names and source movies':'150+ original glyph segments, original choices and source movies');
 await ready();assert.ok(report.musicDecoded&&report.voiceDecoded);pass('Original ADX voice and music decode');
 const activity=await read('activity'),before=await read();
 if(unicode){
  assert.ok(activity.backlog.length>=150);assert.equal(activity.sessions.reduce((n,s)=>n+s.characters,0),activity.backlog.filter(p=>p.kind==='text').reduce((n,p)=>n+characterCount(p.text),0));
  await page.locator('#copyButton').click();assert.equal(await page.evaluate(()=>navigator.clipboard.readText()),plainText(before.state.pending.text));pass('Clipboard and activity count source text, excluding ruby readings');
  await page.locator('#sentence').evaluate(el=>{const range=document.createRange();range.selectNodeContents(el);getSelection().removeAllRanges();getSelection().addRange(range);});await page.locator('#sentence').click();assert.equal((await read()).state.pending.id,before.state.pending.id);await page.evaluate(()=>getSelection().removeAllRanges());pass('Selecting/clicking Japanese does not advance');
 ...
 await page.reload();await ready();assert.equal((await read()).state.pending.id,before.state.pending.id);if(unicode){assert.equal((await read('activity')).sessions.reduce((n,s)=>n+s.characters,0),activity.sessions.reduce((n,s)=>n+s.characters,0));await page.waitForTimeout(300);assert.equal(events.length,eventCount);}pass('Reload restores position without recounting or publishing');
 ...
 await page.locator('#savesButton').click();await page.locator('.slot').filter({hasText:/^slot 1 ·/}).getByRole('button',{name:'Load',exact:true}).click();await ready();assert.equal((await read()).state.pending.id,before.state.pending.id);pass('15 slots; source state restored after advancement');
 await page.locator('#nextButton').click();await ready();await page.locator('#previousButton').click();await ready();assert.equal((await read()).state.pending.id,before.state.pending.id);pass('Previous line restores source state');
 ...
 await page.setViewportSize({width:915,height:412});await page.locator('#fullscreenButton').click();await page.waitForFunction(()=>document.fullscreenElement);const b=await page.locator('#stage').boundingBox();assert.ok(b.y+b.height<=412.5);await page.screenshot({path:path.join(out,'fullscreen.png')});pass('Landscape fullscreen fits');
