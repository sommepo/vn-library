// Actual Never7 import in a temporary server/browser profile; no live bank.
...
const root=path.resolve(import.meta.dirname,'..'),library=path.resolve(process.argv[2]),out=path.resolve(process.argv[3]),game='never7-slps25256-1.01';
...
try{
 ...
 assert.ok(report.voiceSegments>0);await page.waitForFunction(()=>[...window.testAudio].some(a=>a.src.includes('/audio/music/')&&a.readyState>=3&&!a.paused&&a.currentTime>0),null,{timeout:20000});pass('Original-bank BGM plays');
 ...
 await page.locator('#pauseButton').click();await page.waitForFunction(()=>[...window.testAudio].some(a=>a.src.includes('/audio/music/')&&!a.paused));pass('Global pause freezes audio and activity, then resumes');
