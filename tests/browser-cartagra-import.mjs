...
try{
 ...
 await page.getByText('PS2 Cartagra SLPM-66231 v1.01',{exact:true}).waitFor();
 ...
 assert.equal(job.status,'complete');assert.equal(job.gameId,'cartagra-slpm66231-1.01');
 const validation=JSON.parse(await fs.readFile(path.join(state,'imports',jobId,'validation.json')));
 assert.equal(validation.scriptValidation.unsupported,1);assert.equal(validation.scriptValidation.unresolvedReferences,0);
 ...
 await page.waitForFunction(()=>!document.querySelector('#nextButton').disabled||document.querySelector('video.script-media'),null,{timeout:45000});
 assert.ok((await page.locator('#sentence').textContent()).trim()||await page.locator('video.script-media').count());pass('Open game starts the newly installed original story');assert.deepEqual(report.errors,[]);
